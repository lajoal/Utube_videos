...
Use the self-check schema when another tool needs to consume `self_check_summary.json` as a top-level pipeline status object with timing data and artifact presence flags.
...
Use the `sample` files to see clean runs and the `fail.sample` files to see how missing targets, validation issues, top-level self-check step failures, timing data, and artifact presence flags appear in the artifacts.
...
The self-check JSON summary is best for machine-readable orchestration, debugging, runtime analysis, and artifact presence checks.
